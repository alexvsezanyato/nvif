...
vim.api.nvim_create_user_command('Send', function()
  local location = vim.api.nvim_buf_get_var(0, 'location') -- url
  local remotePath = vim.api.nvim_buf_get_var(0, 'remote') -- full remote file path
  local localPath = vim.api.nvim_buf_get_name(0) -- full temporary file path
  ...
  if remotePath == '' or remotePath == nil then
    print('Remote file path is not defined')
    return
  end

  if localPath == '' or localPath == nil then
    print('Local file path is not defined')
    return
  end

  vim.api.nvim_command(
    string.format(
      ':!fish ~/.lftp/edit/send.fish --local=%s --remote=%s --location=%s', 
      localPath,
      remotePath, 
      location
    )
